...
export default function AppProvider({ children }) {
  ...
  // Removed: isLoading state. Suspense will handle loading for lazy components.
  ...
  const actions = useMemo(() => ({
    ...
    // Removed isLoading and setTimeout, relying on React.Suspense
    navigate: (page) => {
  ...
  }), [currentPage, dialog.onConfirm]); // Removed isLoading from dependencies

  // Memoized state values to avoid unnecessary renders on consumer components
  const state = useMemo(() => ({
    submittedArtists,
    currentPage,
    // Removed isLoading from state
    filters,
    dialog
  }), [submittedArtists, currentPage, filters, dialog]); // Removed isLoading from dependencies
